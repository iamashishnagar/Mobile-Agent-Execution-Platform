...
/**
 * Mobile.Inject reads a given agent class from local disk, instantiates a new object from it, and transfers this agent
 * to a given destination IP where the agent starts with the init( ) function.
 *
...
public class Inject {
    ...
    public static String usage = "usage: java -cp Mobile.jar Mobile.Inject host port agent (arg1...N)";

    /**
     * main( ) read an agent byte code from the local disk, instantiates a new object from it, and transfers this agent
     * to a given desgtination IP where the agent starts with init( ) function.
     *
     * @param args consists of host name, port, agent class name, and 0 or more arguments passed to an agent to be
     *             injected.
     */
    ...
    public static void main(String[] args) {
        ...
        String[] arguments = (args.length == 3) ? null : new String[args.length - 3];
        if (arguments != null)
        ...
        try {
            ...
            // let this new agent hop to hostname:port
            agent.setPort(port);
